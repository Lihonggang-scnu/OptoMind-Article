...
\hypersetup{
  pdftitle={Computational Design and Robustness Analysis of Broadband Anti-Reflection Coatings on Fused Silica},
  pdfauthor={Author information pending}
}

\title{Computational Design and Robustness Analysis of Broadband Anti-Reflection Coatings on Fused Silica}
...
\begin{abstract}
The computational study addressed the declared planar multilayer coating objective from 450 to 700 nm at incidence angles 0, 30, 45 degrees for TE and TM polarizations. The study compared bounded structural routes with a common transfer-matrix objective and retained a decision portfolio rather than a single unqualified optimum. For the best-performance candidate, mean R was 0.458\%-1.699\% across the assessed channels (soft target at most 0.800\%); worst case R was 0.784\%-3.697\% across the assessed channels (soft target at most 3.000\%). The retained robust candidate had a robustness score of 0.7455; target domain max R had mean 4.876\% and standard deviation 1.199\%; target domain mean R had mean 1.017\% and standard deviation 0.201\%; target domain min R had mean 0.015\% and standard deviation 0.030\%. The best-performance design satisfied 6 of 12 soft target clauses. The result is therefore reported as a verified best-effort trade-off rather than exact fulfillment of every requested goal. The calculation separates nominal target fit, numerical perturbation sensitivity, and structural simplicity while keeping fabrication claims outside the evidence boundary.
\end{abstract}

\noindent\textbf{Keywords:} thin-film optics, transfer-matrix method, inverse design, robust optimization
...
Forward calculations used the scattering-matrix (S-matrix) formulation over 1001 wavelength samples and 6 angle-polarization channels; the selected design carried a passing physics-acceptance certificate. The bounded comparison completed 3 structural routes with layer counts 4, 7, 10 and produced 27 physically valid candidates. Candidate responses were recomputed with the same forward solver after search, and only records that passed the solver's physical audit were retained for tabulation. The bounded route formulation and perturbation interpretation follow methodological ideas from published multilayer design and tolerance studies \citep{corpusid_120897377_b2aeac7c} \citep{corpusid_284130796_49adbe34} \citep{corpusid_283155301_20887897}. Those sources guide route formulation and tolerance interpretation; they do not certify the simulated performance of the new candidates.
...
\begin{figure}
\hypertarget{fig:visual_01}{%
\centering
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/01_tmm_verified_spectrum.png}
\caption{Calculated spectral response of the best-performance multilayer candidate across all declared angle and polarization channels.}\label{fig:visual_01}
}
\end{figure}
...
\begin{figure}
\hypertarget{fig:visual_02}{%
\centering
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/02_tmm_candidate_portfolio.png}
\caption{Verified performance-robustness portfolio. Marker size increases with the number of finite coating layers.}\label{fig:visual_02}
}
\end{figure}

\FloatBarrier

\hypertarget{best-performance-layer-prescription}{%
\subsection{Best-performance layer prescription}\label{best-performance-layer-prescription}}
...
\hypertarget{verified-objective-values}{%
\subsection{Verified objective values}\label{verified-objective-values}}
...
\hypertarget{candidate-portfolio}{%
\subsection{Candidate portfolio}\label{candidate-portfolio}}
...
\hypertarget{robustness-under-manufacturing-uncertainty}{%
\section{Robustness under Manufacturing Uncertainty}\label{robustness-under-manufacturing-uncertainty}}
...
\begin{figure}
\hypertarget{fig:visual_03}{%
\centering
\includegraphics[width=0.9\textwidth,height=\textheight]{figures/03_tmm_robustness_distribution.png}
\caption{Distribution of the aggregate soft score under the declared thickness and incidence-angle perturbations.}\label{fig:visual_03}
}
\end{figure}
